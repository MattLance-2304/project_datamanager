...
function idbCommitFileData(fileData, p)
{
	...
	store.put({ type: "file", contents:contents, inodeId: fileData.inodeId, permType: fileData.permType }, filePath);
...
function socketReadAsync(fileData, fileOffset, buf, off, len, flags, p)
{
	...
	while(ret < len){
		...
		var l=d.length < len ? d.length : len;
		for(var i=0;i<l;i++)
			buf[off+i|0] = d[i];
		ret = ret + l|0;
		if(d.length == l){
			...
			fileData.chunks[0] = d.subarray(l);
		}
